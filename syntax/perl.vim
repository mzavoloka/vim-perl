...
""" mzavoloka/vim-perl code
syn sync maxlines=500
"syn match perlOps		 "$[\\\"\[\]'&`+*.,;=%~!?@#$<>(-]"
"hi def link perlOps		Operator
syn match perlVarSymbol "[@$%&#]" contained
" NOTE 'Define' is blue in molokai
hi def link perlVarSymbol		Define

" Special variables first ($^A, ...) and ($|, $', ...)
syn match  perlVarPlain		 "$^[ACDEFHILMNOPRSTVWX]\="                                   contains=perlVarSymbol
syn match  perlVarPlain		 "$[\\\"\[\]'&`+*.,;=%~!?@#$<>(-]"                            contains=perlVarSymbol
syn match  perlVarPlain		 "@[-+]"                                                      contains=perlVarSymbol
syn match  perlVarPlain		 "$\%(0\|[1-9]\d*\)"                                          contains=perlVarSymbol
" Same as above, but avoids confusion in $::foo (equivalent to $main::foo)
syn match  perlVarPlain		 "$::\@!"                                                     contains=perlVarSymbol
" These variables are not recognized within matches.
syn match  perlVarNotInMatches	 "$[|)]"                                                  contains=perlVarSymbol
" This variable is not recognized within matches delimited by m//.
syn match  perlVarSlash		 "$/"                                                         contains=perlVarSymbol

" And plain identifiers
syn match  perlPackageRef	 "[$@#%*&]\%(\%(::\|'\)\=\I\i*\%(\%(::\|'\)\I\i*\)*\)\=\%(::\|'\)\I"me=e-1 contained contains=perlVarSymbol
" NOTE 'Nonexisted' is not in molokai, thus should be without coloring (usually white)
hi def link perlPackageRef		Nonexistent

syn match  perlVarPlain  "\%([$@%&]\)\$*\%(\I\i*\)\=\%(\%(::\|'\)\I\i*\)*\%(::\|\i\@<=\)" contains=perlPackageRef,perlVarSymbol nextgroup=perlVarMember,perlVarSimpleMember,perlPostDeref
"syn match  perlVarPlain  "\%([$@%&]\)\$*\%(\I\i*\)\=\%(\%(::\|'\)\I\i*\)*\%(::\|\i\@<=\)"hs=s+1 contains=perlPackageRef nextgroup=perlVarMember,perlVarSimpleMember,perlPostDeref
""" the rest of file is copy of /usr/share/vim90/syntax/perl.vim (and is not loaded bcs of finish just below)
